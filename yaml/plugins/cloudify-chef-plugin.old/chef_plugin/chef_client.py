...
class ChefManager(object):
    ...
    def can_handle(cls, ctx):
        ...
        return all([
                   ctx.properties['chef_config'].get(arg) is not None
    ...
    def install(self):
        ...
        with RetryingLock(ctx, *CHEF_INSTALL_LOCK):
            ...
            try:
                ...
                raise RuntimeError("Chef install failed on:\n%s" % exc)
    ...
    def uninstall(self):
        ...
        if apt_platform():
            ...
            try:
                ...
                raise RuntimeError("chef-client uninstall failed on:\n%s" % exc)
        else:
            ctx.logger.error(
    ...
    def run(self, runlist, chef_attributes):
        ...
        try:
            ...
            raise RuntimeError("The chef run failed\n"
                            "runlist: {0}\nattributes: {1}\nexception: \n{2}".
...
class ChefSoloManager(ChefManager):
    ...
    def _url_to_dir(self, url, dst_dir):
        ...
        try:
            ...
            raise RuntimeError("Failed to extract file {0} to directory {1} "
    ...
    def _get_cmd(self, runlist):
        ...
        if (ctx.properties['chef_config'].get('environment', '_default')
            ...
            if map(int, v.split('.')) < ENVS_MIN_VER:
                raise RuntimeError("Chef solo environments are supported "
...
def get_manager(ctx):
    ...
    raise RuntimeError("Failed to find appropriate Chef manager "
                    "for the specified arguments ({0}). "
                    "Possible arguments sets are: {1}"
                    .format(ctx.properties['chef_config'], arguments_sets))
...
def _context_to_struct(ctx):
    ...
    if hasattr(ctx, 'blueprint_id'):
        ret['blueprint_id'] = ctx.blueprint_id
    if hasattr(ctx, 'deployment_id'):
        ret['deployment_id'] = ctx.deployment_id
...
def _prepare_chef_attributes(ctx):
    ...
    if ctx.related:
        chef_attributes['cloudify'][
            'related'] = _context_to_struct(ctx.related)
...
def run_chef(ctx, runlist):
    ...
    ctx.properties.chef_config.chef_attributes can be a dict or a JSON.
    """
